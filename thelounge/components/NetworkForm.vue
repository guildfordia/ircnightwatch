      ...
      <template v-if="store.state.serverConfiguration?.public">
        <template v-if="config?.lockNetwork">
          <div class="connect-row">
            ...
            <div class="input-wrap">
              <!-- label class="tls">
                <input v-model="displayPasswordField" type="hidden" />
                I have a password
              </label -->
            </div>
          </div>
  ...
  setup(props) {
    ...
    const onUsernameChanged = (event: Event) => {
      ...
      if (props.defaults) {
        ...
        props.defaults.nick = username; // Sync nickname with username
        props.defaults.realname = username; // Sync real name with username
      }
    };
