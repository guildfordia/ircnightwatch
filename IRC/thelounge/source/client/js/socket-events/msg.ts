...
socket.on("msg", function (data) {
    ...
    console.log("Raw data object received from the event:", data);

    const receivingChannel = store.getters.findChannel(data.chan);

    console.log("Receiving Channel Object:", receivingChannel);

    if (!receivingChannel) {
        console.warn("No receiving channel found for:", data.chan);
        return;
    }

    let channel = receivingChannel.channel;

    console.log("Extracted Channel Object:", channel);
